...
export class AuthController {
  constructor(private readonly auth: AuthService, private readonly config: ConfigService, private readonly limiter: RateLimitService) {}
  ...
  async refresh(@Req() request: Request, @Res({ passthrough: true }) response: Response) {
    return this.withRefreshCookie(response, await this.auth.refresh(this.cookie(request, 'capstone_refresh')));
  }

  @Post('logout')
  async logout(@Req() request: Request, @Res({ passthrough: true }) response: Response) {
    const result = await this.auth.logout(this.cookie(request, 'capstone_refresh'));
    response.clearCookie('capstone_refresh', { path: '/api/v1/auth' });
    return result;
  }

  @Post('forgot-password')
  forgotPassword(@Body() dto: ForgotPasswordDto) { this.limiter.hit('forgot', dto.email, 3, 15 * 60_000); return this.auth.forgotPassword(dto); }

  @Post('verify-otp')
  verifyOtp(@Body() dto: VerifyOtpDto) { this.limiter.hit('otp', dto.email, 8, 15 * 60_000); return this.auth.verifyOtp(dto); }
  ...
  async googleCallback(@Query('code') code: string, @Query('state') state: string, @Res() response: Response) {
    ...
    const client = this.config.get<string>('CLIENT_URL', 'http://localhost:3000');
    response.redirect(`${client}/auth/google/callback#accessToken=${encodeURIComponent(result.accessToken)}&next=${encodeURIComponent(result.next)}`);
  }

  @Get('me')
  @UseGuards(JwtAuthGuard)
  me(@CurrentUser() user: AuthUser) { return this.auth.profile(user.sub); }

  private withRefreshCookie(response: Response, result: Awaited<ReturnType<AuthService['login']>>) {
    this.setRefreshCookie(response, result.refreshToken);
    const { refreshToken: _private, ...publicResult } = result;
    return publicResult;
  }

  private setRefreshCookie(response: Response, token: string) {
    response.cookie('capstone_refresh', token, {
      httpOnly: true, secure: this.config.get<string>('NODE_ENV') === 'production',
      sameSite: 'lax', path: '/api/v1/auth', maxAge: 30 * 24 * 60 * 60_000,
    });
  }
