...
import { BookOpen, ChevronRight, CircleDollarSign, Eye, FileCheck2, FileUp, LibraryBig, RefreshCw, ShoppingBag, Users } from "lucide-react";
...
type AdminBook = { id: string; title: string; authors: string[]; accessType: string; status: string; readingEnabled: boolean; ebookPrice: number; stock: number; ebookFile?: { originalFileName: string; fileSize?: number; uploadedAt?: string; storageProvider: "R2" } | null };
...
export function AdminDashboard() {
  ...
  const uploadPdf = async (book: AdminBook, file: File | null) => {
    if (!file) return;
    if (book.ebookFile && !window.confirm(`Thay file đọc trực tuyến của “${book.title}”?`)) return;
    const body = new FormData(); body.append("file", file);
    setUploading(book.id); setError(""); setNotice("");
    try {
      const result = await api<{ pdf: AdminBook["ebookFile"] }>(`/admin/books/${encodeURIComponent(book.id)}/pdf`, { method: "POST", body }, true);
      setBooks((items) => items.map((item) => item.id === book.id ? { ...item, ebookFile: result.pdf, readingEnabled: true } : item));
      setNotice(`Đã tải PDF cho “${book.title}” lên Cloudflare R2.`);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Không thể tải PDF lên R2"); }
