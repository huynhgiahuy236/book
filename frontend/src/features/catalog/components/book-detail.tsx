...
export function BookDetail({ bookId }: { bookId: string }) {
  const [book, setBook] = useState<Book | null>(
    () => fallbackBooks.find((item) => item.id === bookId) ?? null,
  );
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      setPageState(
        remoteBook || fallbackBooks.some((item) => item.id === bookId)
          ? "ready"
          : "not-found",
      );
      setUser(profile);
      ...
      if (profile) {
        const permission = await api<BookAccess>(
          `/library/${encodeURIComponent(bookId)}/access`,
          {},
          true,
        ).catch(() => null);
  ...
  if (pageState === "loading" && !book)
    return (
      <main className="detail-empty">
        ...
        <h1>Đang mở trang sách…</h1>
      </main>
    );
  if (pageState === "not-found" || !book)
    return (
      <main className="detail-empty">
        <BookOpen size={38} />
  ...
  const purchase = async (mode: "payos" | "demo") => {
    if (!user) {
      window.location.assign(
        `/auth?next=/books/${encodeURIComponent(book.id)}`,
      );
      return;
    }
    setLoading(mode);
    ...
    try {
      ...
      if (mode === "demo") {
        ...
        if (!payment.checkoutUrl)
          throw new Error("Giao dịch cũ không còn liên kết thanh toán");
  ...
  const submitReview = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const review = await api<Review>(
        `/books/${encodeURIComponent(book.id)}/reviews`,
        {
          method: "POST",
          body: JSON.stringify({
  ...
  return (
    <main className="detail-page">
      <header className="detail-header">
        ...
        <nav>
          <Link href="/">
            <ArrowLeft size={17} /> Nhà sách
          </Link>
          <Link href="/library">
            <Library size={17} /> Thư viện
          </Link>
          {user ? (
            <span>Xin chào, {user.name.split(" ").at(-1)}</span>
          ) : (
            <Link href={`/auth?next=/books/${encodeURIComponent(book.id)}`}>
      ...
      <section className="detail-hero">
        <div className="detail-cover-stage">
          ...
          <span className="detail-format">{book.format}</span>
        </div>
        <div className="detail-copy">
          ...
          <div className="detail-rating">
            ...
            <i />
            <span>ISBN {book.isbn13 ?? "Đang cập nhật"}</span>
          </div>
          <p className="detail-description">
            {book.description ||
              "Một tựa sách được tuyển chọn cho thư viện CapstoneBook. Nội dung và quyền đọc được quản lý tập trung từ cơ sở dữ liệu."}
          </p>
          <dl>
            <div>
              <dt>Nhà xuất bản</dt>
              <dd>{book.publisher || "Đang cập nhật"}</dd>
            </div>
            <div>
              <dt>Định dạng</dt>
              <dd>{book.format}</dd>
            ...
            <div>
              <dt>Đọc online</dt>
              <dd>
                {book.readingEnabled ? "Có bản PDF bảo vệ" : "Chưa hỗ trợ"}
              </dd>
            </div>
          </dl>
          <div className="detail-purchase">
            <div>
              ...
              {access?.canRead && (
                <small className="detail-owned-note">
                  <ShieldCheck size={14} /> Bạn có quyền đọc Ebook này
                </small>
              )}
            </div>
            {!user ? (
              <Link
                ...
                href={`/auth?next=/books/${encodeURIComponent(book.id)}`}
              >
              ...
              >
                <Eye size={18} />{" "}
                {(access?.progress.currentPage ?? 1) > 1
                  ? `Tiếp tục đọc – Trang ${access?.progress.currentPage}`
                  : "Đọc sách ngay"}
              </Link>
            ) : (
              <div className="detail-actions">
                <button
                  ...
                  onClick={() => void purchase("payos")}
                >
                  <CreditCard size={18} />
                  {loading === "payos" ? "Đang tạo PayOS…" : "Mua qua PayOS"}
                </button>
                {process.env.NODE_ENV === "development" && (
                  <button
                    className="button button-secondary"
                    disabled={loading !== null}
                    onClick={() => void purchase("demo")}
                  >
                    {loading === "demo" ? "Đang cấp quyền…" : "Cấp quyền local"}
                  </button>
                )}
              </div>
            )}
          ...
          <div className="detail-assurance">
            <span>
              <ShieldCheck size={17} /> Webhook PayOS xác nhận quyền
            </span>
            <span>
              <Check size={17} /> Tiến độ lưu theo tài khoản
            </span>
            <span>
              <Check size={17} /> Watermark cá nhân khi đọc
            </span>
          </div>
        </div>
      </section>
      <section className="detail-reviews">
        <header>
          <div>
            <span className="eyebrow">
              <MessageSquare size={14} /> Cộng đồng đọc
            </span>
          ...
          <form onSubmit={submitReview}>
            ...
            <label>
              ...
              <textarea
                ...
                maxLength={1200}
                required
                placeholder="Chia sẻ điều bạn tâm đắc…"
              />
            </label>
        ...
        {reviews.length ? (
          ...
          <div className="empty-state">
            ...
            <p>Hãy là người đầu tiên chia sẻ cảm nhận sau khi sở hữu sách.</p>
          </div>
        )}
      </section>
    </main>
