...
type LibraryItem = { book: { id: string }; progress: { progressPercentage?: number; percent?: number } };
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      if (profile) {
        ...
        if (active) setOwned(library.some((item) => item.book.id === bookId));
      }
    });
    return () => { active = false; };
  }, [bookId]);
