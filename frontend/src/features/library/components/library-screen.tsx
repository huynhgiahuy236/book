...
type LibraryItem = {
  right: { source: "PURCHASE" | "DEMO" | "ADMIN"; createdAt?: string };
...
export function LibraryScreen() {
  ...
  return <main className="library-page">
    <header className="library-header"><Link href="/" className="brand"><span className="brand-mark"><BookOpen size={20}/></span><span>Capstone<span>Book</span></span></Link><nav><Link href="/">Nhà sách</Link>{user?.role === "ADMIN" && <Link href="/admin"><ShieldCheck size={17}/> Quản trị</Link>}<span>{user?.name}</span><button onClick={() => { void api("/auth/logout", { method: "POST" }).finally(() => { clearToken(); window.location.assign("/"); }); }}><LogOut size={17}/> Đăng xuất</button></nav></header>
    ...
    <section className="library-content"><div className="section-head"><div className="section-intro"><span className="eyebrow">Bộ sưu tập</span><h2>Sách của tôi</h2></div><span className="result-count"><ShieldCheck size={14}/> Quyền đọc từ backend</span></div>
      {status === "error" ? <div className="empty-state"><h3>Chưa tải được thư viện</h3><p>Kiểm tra API local rồi tải lại trang.</p></div> : items.length === 0 ? <div className="empty-state"><Library size={30}/><h3>Thư viện đang trống</h3><p>Chưa có Ebook nào được cấp quyền cho tài khoản này.</p><Link className="button button-primary" href="/#books">Khám phá sách</Link></div> : <div className="library-grid">{items.map((item) => { const progress = item.progress.progressPercentage ?? item.progress.percent ?? 0; const currentPage = item.progress.currentPage ?? 1; return <article className="library-card" key={item.book.id}><div className="library-cover" style={{backgroundImage:`url("${item.book.coverUrl}")`}}><span>{item.right.source === "ADMIN" ? "QUYỀN ADMIN" : item.right.source === "DEMO" ? "LOCAL DEMO" : "ĐÃ MUA"}</span></div><div className="library-card-copy"><span>{item.book.categories[0] ?? "Ebook"}</span><h3>{item.book.title}</h3><p>{item.book.authors[0] ?? "Chưa rõ tác giả"}</p><div className="library-progress-label"><span>{progress ? `Trang ${currentPage}` : "Chưa bắt đầu"}</span><strong>{progress}%</strong></div><div className="library-progress-bar"><i style={{width:`${progress}%`}}/></div><Link className="button button-secondary" href={`/read/${encodeURIComponent(item.book.id)}`}>{progress ? `Đọc tiếp · Trang ${currentPage}` : "Bắt đầu đọc"}<ArrowRight size={17}/></Link></div></article>; })}</div>}
    </section>
  </main>;
}
