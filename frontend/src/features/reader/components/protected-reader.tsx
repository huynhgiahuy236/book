...
type ReaderStatus =
  "loading" | "ready" | "unauthorized" | "forbidden" | "missing" | "error";
...
export function ProtectedReader({ bookId }: { bookId: string }) {
    ...
    const load = async () => {
      ...
      try {
        ...
        const savedPage = Math.min(
          Math.max(payload.progress.currentPage ?? 1, 1),
          document.numPages,
        );
        setSession(payload);
        setPdf(document);
        ...
        setMessage(
          cause instanceof Error ? cause.message : "Không thể mở tài liệu",
        );
      }
    };
  ...
  const renderPage = useCallback(async () => {
    ...
    const stageWidth = Math.min(
      Math.max(stageRef.current.clientWidth - 56, 280),
      920,
    );
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      void renderPage().catch((cause) => {
        setMessage(
          cause instanceof Error ? cause.message : "Không thể dựng trang PDF",
        );
        setStatus("error");
      });
    });
  ...
  const saveProgress = useCallback(
    async (keepalive = false) => {
      ...
      if (keepalive) {
        ...
        if (!token) return;
        await fetch(
          `${API_URL}/library/${encodeURIComponent(bookId)}/progress`,
          {
            method: "PATCH",
            headers: {
      ...
      try {
        await api(
          `/library/${encodeURIComponent(bookId)}/progress`,
  ...
  useEffect(() => {
    if (!pdf || pageNumber !== pdf.numPages || status !== "ready") return;
    const timer = window.setTimeout(() => void saveProgress(), 15_000);
    return () => window.clearTimeout(timer);
  }, [pageNumber, pdf, saveProgress, status]);
  ...
  useEffect(() => {
    document
      .querySelector(`[data-reader-page="${pageNumber}"]`)
  ...
  if (status === "loading") {
    return (
      <ReaderGate
        icon={<RefreshCw className="spin" size={28} />}
        title="Đang chuẩn bị trang đọc"
        detail="Đang xác minh quyền và tải tài liệu bảo vệ…"
      />
    );
  }
  if (status !== "ready" || !session || !pdf) {
    ...
    return (
      <ReaderGate
        ...
        title={title}
        detail={
          message || "Vui lòng quay lại trang sách để kiểm tra quyền truy cập."
        }
  ...
  return (
    ...
    >
      <header className="pdf-toolbar">
        <div className="pdf-toolbar-group">
          ...
          <button
            ...
            aria-label="Bật tắt danh sách trang"
          ...
          <div className="pdf-title">
            ...
            <span>{session.book.authors[0] ?? "Ebook"}</span>
          </div>
        </div>
        <div className="pdf-toolbar-group pdf-page-control">
          <button
          ...
          <label>
            ...
            <input
              ...
              onBlur={() => goTo(Number(pageInput) || pageNumber)}
              onKeyDown={(event) => {
                if (event.key === "Enter")
                  goTo(Number(pageInput) || pageNumber);
              }}
            />
          </label>
        ...
        <div className="pdf-toolbar-group pdf-tools">
          <span className={`reader-save-state ${saveStatus}`}>
            <SaveIcon size={14} />
            {saveLabel}
          </span>
          <button
      ...
      <div className="pdf-workspace">
        <aside className={`pdf-sidebar ${sidebarOpen ? "open" : ""}`}>
          <header>
            <div>
              <span>Mục lục trang</span>
              <strong>{pdf.numPages} trang</strong>
            </div>
            <button
          ...
          <div className="pdf-thumbnails">
            {visiblePages[0] > 1 && (
              <button className="pdf-page-gap" onClick={() => goTo(1)}>
                Trang 1 ···
              </button>
            )}
            {visiblePages.map((page) => (
              ...
              >
                <PageThumbnail pdf={pdf} pageNumber={page} />
                <small>Trang {page}</small>
              </button>
            ))}
            {lastVisiblePage < pdf.numPages && (
              <button
                className="pdf-page-gap"
                onClick={() => goTo(pdf.numPages)}
              >
                ··· Trang {pdf.numPages}
              </button>
            )}
        ...
        <section className="pdf-stage" ref={stageRef} aria-busy={rendering}>
          <div className="pdf-canvas-shell">
            {rendering && (
              <div className="pdf-skeleton" aria-label="Đang dựng trang PDF" />
            )}
            <canvas ref={canvasRef} />
            <div className="pdf-watermarks" aria-hidden="true">
              {Array.from({ length: 9 }, (_, index) => (
                <span key={index}>
                  {watermark}
                  <small>
                    {new Date(session.watermark.issuedAt).toLocaleString(
                      "vi-VN",
                    )}
                  </small>
                </span>
      ...
      <footer className="pdf-statusbar">
        <span>
          <ShieldCheck size={15} /> Bản đọc cá nhân có watermark
        </span>
        <strong>
          Trang {pageNumber}/{pdf.numPages} · {progress}%
        </strong>
        <span>Phím ← → để chuyển trang</span>
      </footer>
    </main>
  );
}
...
function ReaderGate({
  ...
  return (
    <main className="reader-gate">
      ...
      <span className="eyebrow">Nội dung được bảo vệ</span>
      <h1>{title}</h1>
      <p>{detail}</p>
